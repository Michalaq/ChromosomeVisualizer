...
public:
	VizWidget(QWidget * widget = nullptr);
	virtual ~VizWidget();
    ...
    enum Action {move, rotate, scale, none};
...
private:
    ...
    QVector3D eye, x, y, z, rotcenter;

    Action a;

public slots:
    void __rotate(int dx, int dy);
    void __scale(int dx, int dy);
    void __move(int dx, int dy);
    void showFrame(int);
};
