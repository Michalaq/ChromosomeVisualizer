...
VizWidget::VizWidget(QWidget * widget)
...
{
	const auto row = modelViewProjection.row(2);

	auto sorter = [&](const QVector4D & a, const QVector4D & b) -> bool {
		auto diff = QVector4D((a - b).toVector3D(), 1.f);
		float z = QVector4D::dotProduct(row, diff);
		return z > 0.f;
	};
...
void VizWidget::keyPressEvent(QKeyEvent *ev)
{
...
void VizWidget::keyReleaseEvent(QKeyEvent *ev)
{
    QOpenGLWidget::keyReleaseEvent(ev);

    if (ev->key() == Qt::Key_M || ev->key() == Qt::Key_R || ev->key() == Qt::Key_S)
        a = none;
}

void VizWidget::mousePressEvent(QMouseEvent *ev)
{
...
void VizWidget::mouseMoveEvent(QMouseEvent *ev)
{
    ...
    {
    case move:
        __move(ev->globalX() - ref.x(), ev->globalY() - ref.y());
        break;
    case rotate:
        __rotate(ev->globalX() - ref.x(), ev->globalY() - ref.y());
        break;
    case scale:
        __scale(ev->globalX() - ref.x(), ev->globalY() - ref.y());
        break;
    case none:
        break;
    }
...
void VizWidget::mouseReleaseEvent(QMouseEvent *ev)
{
...
void VizWidget::wheelEvent(QWheelEvent *ev)
{
    QOpenGLWidget::wheelEvent(ev);

    __scale(ev->angleDelta().y(), ev->angleDelta().y());
}

void VizWidget::__move(int dx, int dy)
{
    eye += x * 0.02 * dx;
    eye -= y * 0.02 * dy;

    mvm.translate(x * 0.02 * dx - y * 0.02 * dy);
...
void VizWidget::__rotate(int dx, int dy)
{
    h -= 0.05 * dx;
    p -= 0.05 * dy;
    ...
    QQuaternion q = QQuaternion::fromAxisAndAngle(y, h);
    QQuaternion r = QQuaternion::fromAxisAndAngle(y, -0.05 * dx);

    x = q.rotatedVector(x);
    z = q.rotatedVector(z);
    eye = rotcenter + r.rotatedVector(eye - rotcenter);

    q = QQuaternion::fromAxisAndAngle(x, p);
    r = QQuaternion::fromAxisAndAngle(x, -0.05 * dy);

    y = q.rotatedVector(y);
    z = q.rotatedVector(z);
    eye = rotcenter + r.rotatedVector(eye - rotcenter);

    q = QQuaternion::fromAxes(x, y, z);

    mvm = QMatrix4x4(q.toRotationMatrix()).inverted();
    mvm.translate(eye);
...
void VizWidget::__scale(int dx, int dy)
{
    Q_UNUSED(dy)
    eye += z * 0.01 * dx;

    mvm.translate(z * 0.01 * dx);

    setModelView(mvm);
    update();
}

void VizWidget::showFrame(int n)
{
    setFrame(n);
    update();
}
