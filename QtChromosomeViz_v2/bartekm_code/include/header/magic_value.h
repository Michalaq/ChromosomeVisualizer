 ...
 * The magic value itself is passed as the return value of a static
 * constexpr method of the Derived subclass.
 * It is required to avoid clashes with the One Definition Rule
 * which severely restricts the other possibilities of passing a constexpr
 * string literal in a header-only library such as protostream:
 *      * a static data member must be also defined/declared out-of-class
 *      * passing the literal as a const char* template parameter technically
 *        means that the deriving class may have a different base
 *        (magic_value<...>) in every translation unit, because the address
 *        of the string literal may vary. Although this code will probably
 *        behave correctly nevertheless, the standard declares the behaviour
 *        undefined.
 */
