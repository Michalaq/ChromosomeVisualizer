...
void MainWindow::captureMovie()
{
    ...
    for (int i = 1; ; i++)
    {
        MovieMaker::captureScene(ui->scene, ui->camera, QString::number(i).rightJustified(QString::number(frames).length(), '0'), ip, ui->horizontalSlider_2->getLowerBound() + i - 1, i, frames);
        if (i != frames)
