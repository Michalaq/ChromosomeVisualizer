...
int Preferences::bt2typename(int bt)
{
    auto i = bt2tn.find(bt);
    return i != bt2tn.end() ? i.value() : 0;
}

int Preferences::ev2typename(std::vector<int> ev)
{
    auto i = ev2tn.find(ev);
    return i != ev2tn.end() ? i.value() : 1;
}

int Preferences::rs2typename(std::string rs)
{
    auto i = rs2tn.find(rs);
    return i != rs2tn.end() ? i.value() : 2;
}

const QString& Preferences::typename2label(int tn)
{
    return tn2defaults[tn].first;
}

const QVariant& Preferences::typename2color(int tn)
{
